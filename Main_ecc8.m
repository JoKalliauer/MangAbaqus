...
%university:TU Wien
 ...
 format longG
  ...
  modelprops.numofelm = 200;
  
  epsil = 0.02;  % finite difference step %epsil = 0.005;
  sortType = 'none'; % eigenvectors sorting type: 'none', 'forwards', 'backwards'
  plotfig= [2,4,7,14,26]; %#ok<*NBRAK>
  forcedeig = []; %1; % forced eigenvector number 'none' sorting
 
  modelprops.elementtype = eltype;
  
  %modelprops.lambda = 5*epsil; % do not go over snap-through point
  modelprops.lambda = 4*epsil:epsil:.42;%3.725;%10;%2.19;%1.5;%2.169;%3.72; %0:0.01:5.68 %(0.78-4*epsil); % do not go over snap-through point 5*epsil:10*epsil:(0.78-4*epsil)
