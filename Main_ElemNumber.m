...
%university:TU Wien
 ...
 % close all
  ...
  modelprops.numofelm = 1000;
  
  epsil = 0.05;  % finite difference step %epsil = 0.005;
  sortType = 'none'; % eigenvectors sorting type: 'none', 'forwards', 'backwards'
  plotfig= [2,12,-2,14,-14]; %#ok<*NBRAK>
  forcedeig = []; %1; % forced eigenvector number 'none' sorting
 
  modelprops.elementtype = eltype;
  
  %modelprops.lambda = 5*epsil; % do not go over snap-through point
  modelprops.lambda = 0:0.1:1.19; %(0.78-4*epsil); % do not go over snap-through point 5*epsil:10*epsil(0.78-4*epsil)
  ...
  main.check=false;
  main.colorshift=modelprops.numofeigs*2;
