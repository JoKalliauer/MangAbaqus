...
diagramname='wichtig/eccEWH5mm.csv';
...
%system(['exec sed -i "s/,/\./g" ',filename])
 ...
 myVec=[2,3,6,4,4,7:9];
 yMat(Pos1,1:NRSpalten)=PT01(:,myVec(1:NRSpalten));
 zMat=0*yMat;
 %xMat=zMat;
 zMat(Pos1,4)=PT01(:,5);
 zMat(Pos1,5)=-PT01(:,5);

 xMattmp=[xValCor,xValCor,xValCor,xValCor,xValCor,xValCor,xValCor,xValCor];
 xMat=xMattmp(:,1:NRSpalten);
 ...
 for i=myivec
  xy2=[xMat(:,i),yMat(:,i)];
  xy2(isnan(xy2(:,2)),1)=NaN;
  xy2d = sortrows(xy2,1,'ascend','MissingPlacement','last');

  xMat2(:,i)=xy2d(:,1);
  yMat2(:,i)=xy2d(:,2);
 end
 zMat2(1:221,myivec)=zeros(221,nrtmp);
 zMat2h=zMat2-.01;
 yMat2h=yMat2-.001;
 xMat2h=xMat2-.001;
