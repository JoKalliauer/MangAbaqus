function [filename,lambda,BC,Nodes,Elements]  = eccenCompressionBeam2D(L,numofelm,lambda,loadFactor,elType,ecc)
if nargin<1
    L = 5.0;
end
if nargin<2
...
filename = ['eccenCompressionBeam2D-',elType,'-',num2str(numofelm(end)),'-len-',num2str(L),'-ecc-',num2str(ecc),'-loadfac-',num2str(loadFactor)];
...
%% Boundary conditions
 BC = [7*(rpLeft1 - 1) + 1, 0
       7*(rpLeft1 - 1) + 2, 0;
       7*(rpRight1 - 1) + 2, 0;
       7*(Nodes(:,1)-1) + 3, zeros(size(Nodes,1),1);
       7*(Nodes(:,1)-1) + 4, zeros(size(Nodes,1),1);
       7*(Nodes(:,1)-1) + 5, zeros(size(Nodes,1),1);
       7*(Nodes(:,1)-1) + 7, zeros(size(Nodes,1),1)];
%%
